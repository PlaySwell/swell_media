	...
	class RootController < SwellMedia::MediaController
		...
		def show
			...
			if @media.slug == 'homepage'
				set_page_meta( title: "#{ENV['APP_NAME']}", twitter: { site: ENV['TWITTER_SITE'], card: twitter_card, description: ENV['TWITTER_HOMEPAGE_DESCRIPTION'] }, og: { type: 'website', image: @media.avatar } )
				...
				set_page_meta( title: "#{@media.sanitized_content[0..128]} | #{ENV['APP_NAME']}", description: @media.content, twitter: { site: ENV['TWITTER_SITE'], card: twitter_card }, og: { description: ActionView::Base.full_sanitizer.sanitize(@media.content), image: @media.avatar } )
			end
